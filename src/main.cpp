...
#if defined(NDEBUG)
# error "Suwoncoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Suwoncoin Signed Message:\n";
...
{
    // Suwoncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Suwoncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Suwoncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Suwoncoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Suwoncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SuwoncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SuwoncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static SuwoncoinMiner(CWallet *pwallet)
{
    ...
    try { loop {
        ...
        printf("Running SuwoncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("SuwoncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&SuwoncoinMiner, pwallet));
}
